...
def run_agent(user_goal: str):
    ...
    for step in range(max_iterations):
        ...
        response = client.chat.completions.create(
            ...
            tool_choice="auto",
            temperature=0.0,
            max_tokens=1500,  # Slightly higher to accommodate structured output
            response_format={
                "type": "json_schema",
                "json_schema": {
...
if __name__ == "__main__":
    run_agent(
        "I want to understand how the speed of sound compares to the speed of light. "
        "Specifically: if a person on the Moon clapped their hands, "
        "(1) how long would it take for the light from that clap to reach Earth, "
        "(2) how long would it take for the sound to reach Earth assuming sound could travel through space "
        "(which it can't, but assume it can at its normal speed), "
        "(3) what's the ratio between those two times, "
        "and (4) explain why this ratio matters — what does it tell us about why we see lightning before we hear thunder? "
        "Use real distances and speeds, look up anything you need."
    )